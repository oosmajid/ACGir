...
## 🎬 نصب اختیاری FFmpeg (با یک کلیک)

نیازی نیست FFmpeg را جداگانه نصب کنی. اگر روی سیستم نصب نباشد، خود برنامه در صفحهٔ اصلی یک کارت نشان می‌دهد و **با اجازهٔ تو** آخرین نسخهٔ رسمی FFmpeg را دانلود و نصب می‌کند:

- **ویندوز:** بیلد `latest` از [BtbN/FFmpeg-Builds](https://github.com/BtbN/FFmpeg-Builds) (۶۴-بیتی / ARM64).
- **مک:** بیلد ایستا (static) از [evermeet.cx](https://evermeet.cx/ffmpeg/) — روی Intel به‌صورت بومی و روی Apple Silicon از طریق Rosetta اجرا می‌شود.

فایل فقط داخل پوشهٔ کش برنامه ذخیره می‌شود (`~/Library/Caches/acgir/ffmpeg` روی مک، `%LocalAppData%\acgir\ffmpeg` روی ویندوز) و در دانلودِ بعدی به‌صورت خودکار استفاده می‌شود. این کار کاملاً **اختیاری** است؛ برنامه بدون آن هم در حالت بدون-dependency کار می‌کند.
